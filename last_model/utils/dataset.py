...
class tumor(Dataset):
    ...
    def _load(self, file):
        file = [file]
        input,  class_y, masks, name, pixel_sum = [], [], [], [], []
        for i in file:
            if i in self.file_normal:
                ...
                black_img = Image.open('/home/junkyu/project/tumor_detection/densenet_RPN/data/black/'+i[:-4]+'.png').convert('L')
                ...
                for y in range(2):
                    for x in range(4):
                        ...
                        masks.append(black_img)
                        name.append(i)
                        pixel_sum.append(T.ToTensor()(black_img).sum())
        return input, class_y, masks, name, pixel_sum
    ...
    def __getitem__(self, idx):
        ...
        input, class_y, masks, name, pixel = self._load(file)
        if self.transform is not None:
            for i in range(8):
                input[i], masks[i] = self.transform((input[i],masks[i]))
        # input 8개씩
        return input, class_y, masks, name, pixel
