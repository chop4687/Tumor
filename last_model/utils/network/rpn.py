...
class DenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        temp_c2 = self.block1(out)
        temp_c3 = self.block2(temp_c2)
        temp_c4 = self.block3(temp_c3)
        temp_c5 = self.block4(temp_c4)

        c2 = self.maxpool(temp_c2)
        c3 = self.maxpool(temp_c3)
        c4 = self.maxpool(temp_c4)
        c5 = self.maxpool(temp_c5)
        ...
        return cls, bbox, temp_c2, temp_c3, temp_c4, temp_c5
...
def densenet121_RPN(pretrained=False, progress=True, **kwargs):
